...
`
  ...
  // Both names, one value, written here because this is the only place that
  // knows it — the ground is sampled from the artwork rather than declared.
  //
  // splash_background used to be typed by hand in android-setup.mjs as #0F1711,
  // which is the app's dark surface and not the artwork's ground. On Android 12
  // and up the system draws the launch screen itself: the icon, masked to a
  // circle, on splash_background. With the two greens a few values apart that
  // circle was visible as a disc of slightly the wrong colour around the logo.
  // Sampled from the same pixels as the icon's own ground, the seam disappears.
  const values = ensure(path.join(ANDROID_RES, 'values'))
  const hex = `#${DEEP.map(c => c.toString(16).padStart(2, '0')).join('')}`
  fs.writeFileSync(path.join(values, 'brand_ground.xml'), `<?xml version="1.0" encoding="utf-8"?>
<!-- Generated by scripts/make-icons.mjs from the artwork. Do not edit. -->
<resources>
    <color name="ic_launcher_background">${hex}</color>
    <color name="splash_background">${hex}</color>
</resources>
`)
  // The colour used to live here under the icon's name alone; a leftover copy
  // would declare ic_launcher_background twice and the build would refuse it.
  const stale = path.join(values, 'ic_launcher_background.xml')
